...
const VIEWER_URL = chrome.runtime.getURL("content/web/viewer.html");
...
if (document.contentType === "application/pdf") {
  //console.log(`document.contentType === ${document.contentType} inner`)
...
function maybeRenderPdfDoc(isNotPOST) {
  ...
  console.log(`Detected PDF: ${document.contentType}\n${document.URL}`);

  // Create a container for both your component and the PDF viewer
  ...
  // Create your component's iframe (left side - 35%)
  const vueIframe = document.createElement('iframe');
  vueIframe.id = 'vue-iframe'
  vueIframe.style.width = '30%';
  vueIframe.style.height = '100%';
  vueIframe.style.border = 'none';
  vueIframe.src = chrome.runtime.getURL('gptcore/index.html'+"?ts="+vid);
  vueIframe.allow="clipboard-read; clipboard-write"
  container.appendChild(vueIframe);

  // Create PDF viewer iframe (right side - 65%)
  const pdfViewer = document.createElement('iframe');
  pdfViewer.style.flex = 1
  pdfViewer.style.height = '100%';
  pdfViewer.style.border = 'none';
  pdfViewer.src = getEmbeddedViewerURL(document.URL);
  pdfViewer.allow="clipboard-read; clipboard-write"
  // 等待 iframe 加载完成
  pdfViewer.onload = () => {
    // 现在 contentWindow 可用
  ...
  const pdfbar = document.createElement('iframe');
  pdfbar.style.width = '1%';
  pdfbar.style.height = '100%';
  pdfbar.style.border = 'none';
  pdfbar.src = chrome.runtime.getURL('pdfbar/index.html'+"?ts="+vid);
  container.appendChild(pdfbar);
